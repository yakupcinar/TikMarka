...
<template>
  ...
  <PanelDuzeni>
    <Link href="/yonetim/musteriler" class="text-sm text-stone-500 hover:underline">← Müşteriler</Link>

    <h1 class="mt-2 text-2xl font-bold">{{ musteri.ad }}</h1>

    <p class="text-stone-600">
      {{ musteri.eposta }}
      <span v-if="!musteri.dogrulanmis" class="ml-2 rounded bg-amber-100 px-2 py-0.5 text-xs text-amber-800">
    ...
    <div class="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
      <div class="rounded-xl border border-stone-200 p-4">
        ...
        <div class="text-sm text-stone-500">tamamlanan sipariş</div>
      </div>
      ...
      <div class="rounded-xl border border-stone-200 p-4">
        ...
        <div class="text-sm text-stone-500">toplam (iadeler düşülmedi)</div>
      </div>

      <div class="rounded-xl border border-stone-200 p-4">
        <div class="text-2xl font-bold">{{ ozet.favori }}</div>
        <div class="text-sm text-stone-500">favori</div>
      </div>

      <div class="rounded-xl border border-stone-200 p-4">
        <div class="text-2xl font-bold" :class="ozet.basarisiz > 0 ? 'text-red-700' : ''">{{ ozet.basarisiz }}</div>
        <div class="text-sm text-stone-500">başarısız ödeme</div>
      </div>
    </div>
    ...
    <p v-if="siparisler.length > 0" class="text-sm text-stone-500">
      Son {{ siparisler.length }} sipariş — bekleyen ve iptal edilenler dâhil.
    </p>

    <p v-if="siparisler.length === 0" class="text-stone-500">Bu müşterinin siparişi yok.</p>

    <table v-else class="mt-2 w-full text-sm">
      <tbody>
        <tr v-for="s in siparisler" :key="s.uuid" class="border-t border-stone-100">
          <td class="py-3">
            ...
            <div class="text-xs text-stone-500">{{ s.urunler.join(' · ') }}</div>
          </td>
          <td class="text-stone-500">{{ tarih(s.tarih) }}</td>
    ...
    <p v-if="favoriler.length === 0" class="text-stone-500">Favori ürünü yok.</p>

    <ul v-else class="mt-2 space-y-1 text-sm">
      <li v-for="(f, i) in favoriler" :key="i" class="flex gap-3 border-t border-stone-100 py-2">
        <span class="flex-1">{{ f.urun }}</span>
        <span class="text-stone-500">{{ tarih(f.tarih) }}</span>
      </li>
    </ul>
    ...
    <p v-if="basarisizOdemeler.length === 0" class="text-stone-500">Başarısız ödeme denemesi yok.</p>

    <ul v-else class="mt-2 space-y-1 text-sm">
      <li v-for="(o, i) in basarisizOdemeler" :key="i" class="flex gap-3 border-t border-stone-100 py-2">
        <span class="flex-1">{{ o.numara }}</span>
        <span>{{ para(o.tutar) }}</span>
        <span class="text-stone-500">{{ o.saglayici }}</span>
        <span class="text-stone-500">{{ tarih(o.tarih) }}</span>
      </li>
    </ul>
  </PanelDuzeni>
</template>
